...
class LMP:
    ...
    def __init__(self, name, cfg, fixed_vars, variable_vars, debug=False, env=''):
        ...
        if prompt_fname:
            if os.path.isabs(prompt_fname):
                prompt_path = prompt_fname
            else:
                from_lmp_py_dir = os.path.dirname(os.path.abspath(__file__))
                prompt_path = os.path.abspath(os.path.join(
                    from_lmp_py_dir, 'prompts', env, prompt_fname
                ))
            
            if not prompt_path.endswith('.txt'):
                prompt_path += '.txt'
            
            self._base_prompt = load_prompt(prompt_path)
        else:
            self._base_prompt = ''
        ...
        self.exec_hist = ''
        self._context = None
        self._cache = DiskCache(load_cache=self._cfg.get('load_cache', True))
    ...
    def build_prompt(self, query):
        ...
        if self._cfg.get('maintain_session', False) and self.exec_hist != '':
            prompt += f'\n{self.exec_hist}'
        
        prompt += '\n'

        if self._cfg.get('include_context', False):
            assert self._context is not None, 'context is None'
            prompt += f'\n{self._context}'

        user_query = f'{self._cfg.get("query_prefix", "")}{query}{self._cfg.get("query_suffix", "")}'
        prompt += f'\n{user_query}'
    ...
    def __call__(self, query, **kwargs):
        is_condition_checker = kwargs.pop('is_condition_checker', False)
        if is_condition_checker:
        ...
        while True:
            try:
                ...
                code_str = code_str.strip().rstrip('#').strip()
                break
        ...
        if self._name in ['meta_planner', 'condition_checker']:
            return code_str
        ...
        to_exec = '\n'.join([line for line in to_exec.split('\n') if not line.strip().startswith('import')])
        ...
        if not self._name in ['composer', 'planner', 'loco_manip_executor']:
        ...
        if self._cfg.get('maintain_session', False):
            self._variable_vars.update(lvars)

        if self._cfg.get('has_return', False):
